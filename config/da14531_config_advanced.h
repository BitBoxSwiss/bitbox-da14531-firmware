...
/****************************************************************************************************************/
/* Enables True Random Number Generator. A true random number, generated at
 * system initialization, is used to   */
...
/* If the secure connections mode is to be used the macro must be defined. The
 * secure connections mode uses     */
/* private/public keys which have been created based on the Elliptic-curve
 * Diffie-Hellman (ECDH) protocol.      */
/* Note for DA14585/586/531: */
/* If the macro is defined, the ECDH keys will be created only once after the
 * system start-up. If the legacy    */
/* pairing is to be used, it is recommended to undefine the macro in order to
 * gain faster start-up time and     */
/* reduce the RAM footprint. */
/* Note for DA14531-01: */
/* The ECDH keys are always created after a pairing request. If the legacy
 * pairing is to be used, it is         */
/* recommended to undefine the macro in order to reduce the RAM footprint. */
/****************************************************************************************************************/
#define CFG_ENABLE_SMP_SECURE

/****************************************************************************************************************/
/* Uses ChaCha20 random number generator instead of the C standard library
...
/* Custom heap sizes */
/****************************************************************************************************************/
#define DB_HEAP_SZ 600  // default 1024
#define ENV_HEAP_SZ 364 // depends on max connections
// #define MSG_HEAP_SZ 2000 // depends on max connections
#define MSG_HEAP_SZ 5000
#define NON_RET_HEAP_SZ 0 // default 1024 for 1 max connection
 ...
 * a channel                       */
/****************************************************************************************************************/
#define CFG_NVDS_TAG_BD_ADDRESS {0x0D, 0x00, 0xF4, 0x35, 0x23, 0x48}
...
/****************************************************************************************************************/
/* Maximum supported TX data packet length (supportedMaxTxOctets value, as
 * defined in 4.2 Specification).       */
/* Range: 27 - 251 octets. */
/* NOTE 1: Even number of octets are not supported. A selected even number will
 * be automatically converted to   */
/*         the next odd one. */
/* NOTE 2: The supportedMaxTxTime value is automatically calculated by the ROM
...
/****************************************************************************************************************/
/* Maximum supported RX data packet length (supportedMaxRxOctets value, as
 * defined in 4.2 Specification).       */
/* Range: 27 - 251 octets. */
/* NOTE 1: Even number of octets are not supported. A selected even number will
 * be automatically converted to   */
/*         the next odd one. */
/* NOTE 2: The supportedMaxRxTime value is automatically calculated by the ROM
 * code, according to the following */
/*         equation: */
...
/****************************************************************************************************************/
/* Duplicate filter max value for the scan report list. The maximum value shall
 * be 100.                         */
/****************************************************************************************************************/
#define CFG_BLE_DUPLICATE_FILTER_MAX (10)

/****************************************************************************************************************/
/* Duplicate filter flag for the scan report list. This flag controls what will
 * be reported if the              */
/* CFG_BLE_DUPLICATE_FILTER_MAX number is exceeded. */
/*     - If the flag is defined, the extra devices are considered to be in the
 * list and will not be reported.   */
/****************************************************************************************************************/
...
/****************************************************************************************************************/
/* Maximum retention memory in bytes. The base address of the retention data is
 * calculated from the selected    */
/* size. */
/****************************************************************************************************************/
#define CFG_RET_DATA_SIZE (2300)
...
/****************************************************************************************************************/
/* RAM cell(s) retention mode handling. The user has to select which RAM cells
 * must be retained during the      */
/* extended sleep, based on his/her application RAM layout. The last RAM block
 * is always retained, since it     */
...
/****************************************************************************************************************/
/* Non-retained heap handling. The non-retained heap is either empty or not, and
 * it may fill with messages      */
/* during the application runtime. If it is not empty while the system is going
 * to extended sleep, it must be   */
/* retained. Macro state: */
/*      - If the macro is defined then the retention mode of the RAM cell(s),
 * where the non-ret heap resides,   */
/*        is automatically controlled by the SDK. */
/*      - If the macro is undefined then the retention mode of the RAM cell(s),
 * where the non-ret heap resides, */
...
/*     - CFG_CODE_LOCATION_EXT: Code is loaded from SPI flash / I2C EEPROM /
 * UART                               */
/*     - CFG_CODE_LOCATION_OTP: Code is burned in the OTP */
/* The above options are mutually exclusive and exactly one of them must be
 * enabled.                            */
/****************************************************************************************************************/
...
/****************************************************************************************************************/
/* Code size for OTP copy on (extended sleep with OTP copy on). If the OTP copy
 * is on and the default SDK       */
/* scatter file is not used the following macro must define the code size in
 * bytes for the OTP copy.            */
/****************************************************************************************************************/
#undef CFG_CODE_SIZE_FOR_OTP_COPY_ON

/****************************************************************************************************************/
/* Temperature range selection (it applies to hibernation mode only). */
/* - CFG_HIGH_TEMPERATURE:         Device is configured to operate at high
 * temperature range (-40C to +105C).   */
/* - CFG_AMB_TEMPERATURE:          Device is configured to operate at ambient
 * temperature range (-40C to +40C). */
/* - CFG_MID_TEMPERATURE:          Device is configured to operate at mid
 * temperature range (-40C to +60C).     */
/* - CFG_EXT_TEMPERATURE:          Device is configured to operate at ext
 * temperature range (-40C to +85C).     */
/****************************************************************************************************************/
#define CFG_AMB_TEMPERATURE

/****************************************************************************************************************/
/* Disable quadrature decoder on start up. The quadrature decoder is by default
 * enabled on system power up and  */
